...
{  
        ...
        build->sprite.spriteheight = fabs(build->data.res_y / (double)(build->sprite.transformy)); 
        ...
        build->sprite.spritewidth = fabs((build->data.res_y / ( build->sprite.transformy)));
...
static void order(t_build *build)
{
    ...
    if (!(build->sprite.sprite_tex = mlx_xpm_file_to_image(build->img.mlx, path, &width, &height)))
        error("no", 2);
    if(!(build->sprite.dataadres = mlx_get_data_addr(build->sprite.sprite_tex, &build->sprite.bpp,\
    &build->sprite.line_length, &build->sprite.endian)))
        error("no", 2);
    i = 0;
    while( i < build->sprite.num)
    {
        spritedistance[i] = ((build->cor.start_pos_x - build->sprite_s.sprite_cor[i][0]) * \
...
static int loop(t_build *build)
{
    ...
    {
        build->sprite.texx = (int)(256 * (build->sprite.stripe - (-build->sprite.spritewidth \
        / 2 + build->sprite.spritescreenx)) * textwidth / build->sprite.spritewidth / 256);
        if(build->sprite.transformy > 0 && build->sprite.stripe > 0 && build->sprite.stripe \
        < build->data.res_x && build->sprite.transformy < build->sprite.zbuffer[build->sprite.stripe])
        makesprite(build);
        build->sprite.stripe++;
    }
    return(0);
...
void        sprite(t_build *build)
{
    ...
    while(i < build->sprite.num)
    {
        build->sprite.spritex = build->sprite_s.sprite_cor[i][0] - build->cor.start_pos_x + 0.5;
        build->sprite.spritey = build->sprite_s.sprite_cor[i][1] - build->cor.start_pos_y +0.5;
